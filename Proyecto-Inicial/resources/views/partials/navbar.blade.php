
          <ul id="menu">
            <li class="item"><a href="{{url('/ofertas')}}">Ofertas laborales</a></li>
            <li class="item"><a href="{{url('/directorio')}}">Directorio de empresas</a></li>
            <li class="item"><a href="{{url('/ranking')}}">Ranking de empresas</a></li>
            <li class="item"><a href="{{url('/tabuladorSalarios')}}">Tabulador de salarios</a></li>
            <li class="item"><a href="{{url('/eventos')}}">Eventos UTM</a></li>
            <li class="item"><a href="{{url('/historiasExito')}}">Historias de éxito</a></li>
            <li class="item"><a href="{{url('/tipsConsejos')}}">Tips y consejos</a></li>
            <!-- <li class="item"><a href="#">Cerrar sesión</a></li> -->
          </ul>
